...
var _typeTransition = require("./TypeTransition");
...
_utils.preloadImages('.item__img, .article__img').then(()=>document.body.classList.remove('loading')
);
// transição de texto
const typeT = new _typeTransition.TypeTransition(document.querySelector('[data-type-transition]'));
...
[
    ...
    // create um novo Item
    const item = new _item.Item(itemEl);
...
const openItem = (item1)=>{
    if (isAnimating) return;
    isAnimating = true;
    // update currentItem index
    currentItem = itemsInstanceArr.indexOf(item1);
    // gsap timeline
    const openTimeline = _gsapCoreDefault.default.timeline({
        onComplete: ()=>isAnimating = false
    });
    // rótulos
    openTimeline.addLabel('start', 0)// a transição do tipo começa um pouco depois da animação dos itens
    .addLabel('typeTransition', 0.3)// o artigo vai mostrar um pouco antes de terminar a transição do texto
    ...
    .to(itemsInstanceArr.map((item)=>item.DOM.el
        ...
        opacity: 0,
        y: (pos)=>pos % 2 ? '25%' : '-25%'
    }, 'start')// esmaecer o quadro da página
    .to(frameEl, {
        duration: 0.8,
        ease: 'power3',
        opacity: 0,
        onComplete: ()=>_gsapCoreDefault.default.set(frameEl, {
                pointerEvents: 'none'
            })
    }, 'start')// a transição do texto começa aqui
    .add(typeT.in().play(), 'typeTransition')// add a classe atual nos item's article e define os ponteiros do evento
    .add(()=>{
        _gsapCoreDefault.default.set(backCtrl, {
            pointerEvents: 'auto'
        });
        _gsapCoreDefault.default.set(itemsWrap, {
            pointerEvents: 'none'
        });
        itemsInstanceArr[currentItem].DOM.article.classList.add('article--current');
    }, 'articleOpening')// mostre o botão voltar
    .to(backCtrl, {
        duration: 0.7,
        opacity: 1
    }, 'articleOpening')//ocultar inicialmente todos os elementos do artigo para que possamos animá-los em
    .set([
        item1.article.DOM.title,
        item1.article.DOM.number,
        item1.article.DOM.intro,
        item1.article.DOM.description
    ], {
        opacity: 0,
        y: '50%'
    }, 'articleOpening')// a image wrap e image elements  terão valores de tradução opostos (efeito de revelar / não revelar
    .set(item1.article.DOM.imageWrap, {
        y: '100%'
    }, 2).set(item1.article.DOM.image, {
        y: '-100%'
    }, 2)//agora fade in e traduza os elementos do artigo
    .to([
        item1.article.DOM.title,
        item1.article.DOM.number,
        item1.article.DOM.intro,
        ...
        y: '0%',
        stagger: 0.04
    }, 'articleOpening')// e revelar a imagem
    .to([
        item1.article.DOM.imageWrap,
        item1.article.DOM.image
    ], {
        duration: 1,
        ease: 'expo',
        y: '0%'
...
const closeItem = ()=>{
    ...
    isAnimating = true;
    // item aberto atual
    const item2 = itemsInstanceArr[currentItem];
    // gsap Linha do tempo
    const closeTimeline = _gsapCoreDefault.default.timeline({
        onComplete: ()=>isAnimating = false
    });
    // rótulos
    closeTimeline.addLabel('start', 0).addLabel('typeTransition', 0.5).addLabel('showItems', typeT.out().totalDuration() * 0.7 + closeTimeline.labels.typeTransition).to(backCtrl, {
        ...
        item2.article.DOM.title,
        item2.article.DOM.number,
        item2.article.DOM.intro,
        item2.article.DOM.description
    ], {
        duration: 1,
        ease: 'power4.in',
        opacity: 0,
        y: '50%',
        stagger: -0.04
    }, 'start').to(item2.article.DOM.imageWrap, {
        duration: 1,
        ease: 'power4.in',
        y: '100%'
    }, 'start').to(item2.article.DOM.image, {
        duration: 1,
        ease: 'power4.in',
        y: '-100%'
    }, 'start')// remove a classe atual do artigo do item e define os eventos de ponteiro
    .add(()=>{
        _gsapCoreDefault.default.set(backCtrl, {
            pointerEvents: 'none'
        });
        ...
        item2.DOM.article.classList.remove('article--current');
    })// a transição do texto começa aqui
    .add(typeT.out().play(), 'typeTransition').to(frameEl, {
        duration: 0.8,
        ease: 'power3',
        opacity: 1,
        onStart: ()=>_gsapCoreDefault.default.set(frameEl, {
                pointerEvents: 'auto'
            })
    }, 'showItems').to(itemsInstanceArr.map((item)=>item.DOM.el
    ), {
        duration: 1,
        ease: 'power3.inOut',
        opacity: 1,
...
},{"./utils":"fIYUT","gsap":"2aTR0","./TypeTransition":"4Srzg","./Item":"8olma","gsap/gsap-core":"dbpJR","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"fIYUT":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
Tween1.version = Timeline1.version = gsap.version = "3.9.1";
_coreReady = 1;
_windowExists() && _wake();
...
},{"./gsap-core.js":"dbpJR","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"4Srzg":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"gsap":"2aTR0","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"8olma":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Item {
    ...
    constructor(DOM_el){
        ...
        // Article instance
        this.article = new _article.Article(this.DOM.article);
        ...
        this.DOM.el.addEventListener('mouseenter', ()=>{
            _gsapCoreDefault.default.timeline({
                defaults: hoverTimelineDefaults
            }).to([
                this.DOM.image,
                this.DOM.title,
...
},{"gsap/gsap-core":"dbpJR","./Article":"iNJ0W","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"iNJ0W":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
class Article {
    ...
    // DOM  = O Document Object Model ou simplesmente DOM é utilizado pelo navegador Web para representar a sua página Web.
    constructor(DOM_el){
        this.DOM.el = DOM_el;
        // image
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}]},["cSv3F","3auaO"], "3auaO", "parcelRequire469c")
